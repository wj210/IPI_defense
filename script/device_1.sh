  ...
  while true; do
    ...
    while IFS= read -r line; do 
      ...
      if [ "$actual_free_memory" -ge "$REQUIRED_MEMORY" ]; then
        ...
        # srun -p $p -w $w --exact --job-name=t_$gpu_id python mem_new.py --device_no $gpu_id --memory $REQUIRED_MEMORY &
        OCCUPY_SCRIPT_PIDS+=($!)
...
srun -p $p -w $w -c $c --verbose --job-name=self_learning --gpus=$num_gpu bash script/run_1.sh
...
#           srun -p $p -w $w --gpus=1 python mem_new.py --device_no $gpu_id --memory $REQUIRED_MEMORY --device_no $gpu_id &
#           OCCUPY_SCRIPT_PIDS+=($!)
#       done
#       sleep 100000000
#     done
